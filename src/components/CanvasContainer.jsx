...
const CanvasContainer = () => {
  ...
  const canvasRef = useRef(null) // The canvas reference to refer to canvas instance inside the DOM
  const [canvasData, setCanvasData] = useState(null) // The state to hold the canvas data
  const [drawingEnabled, setDrawingEnabled] = useState(true) // The state to keep track of drawing enabled or disabled
  const canvasWidth = 1300 // The canvas width
  const canvasHeight = 910 // The canvas height
  // Task 2 (state variables)
  const [isPanning, setIsPanning] = useState(false) // state to keep track of pan mode enabled or disabled
  const panRef = useRef(false) // The reference to refer to canvas pan inside the DOM
  // Task 6 (state variables)
  const [canvasHistory, setCanvasHistory] = useState([]) // The state to store history of the canvas element
  const [currentHistoryIndex, setCurrentHistoryIndex] = useState(-1) // The state to keep track of the index within the history state
  // Task 9 (state variables)
  const [selectedColor, setSelectedColor] = useState('black') // The state to keep track of currently selected color
  const colors = [
    'red',
    'blue',
    'green',
  ...
  const [selectedBrush, setSelectedBrush] = useState('Pencil brush') // The state to keep track of currently selected brush
  ...
  useEffect(() => {
    ...
    newCanvas.on('mouse:down', function (opt) {
      if (newCanvas.isDrawingMode === true) return
      else {
    ...
    newCanvas.on('mouse:dblclick', function (opt) {
      ...
      const text = new fabric.Textbox(
        'Click to add text when drawing mode is disabled',
        {
          left: x,
          top: y,
          fontSize: 27,
    ...
    if (currentHistoryIndex >= 0) {
      const previousState =
        currentHistoryIndex === 0
          ? canvasHistory[0]
          : canvasHistory[currentHistoryIndex - 1]
  ...
  const save = () => {
    ...
    const jsonBlob = new Blob([JSON.stringify(drawingData)], {
  ...
  const addShape = (shape) => {
    ...
    if (shape === 'Rectangle') {
      ...
      const triangleTop =
        canvasCenterY -
        ((Math.sqrt(3) / 2) * triangleSideLength) / 2 +
        newOffsetY
      const triangle = new fabric.Polygon(
        [
          ...
          {
            x: triangleSideLength / 2,
            y: -(Math.sqrt(3) / 2) * triangleSideLength,
          },
        ],
        {
          left: triangleLeft,
        ...
        <div class="container">
          ...
          <div class="row">
            ...
            <div
              ...
              className={`col selected-color-height`}
            ></div>
          </div>
          {/* Task 11 (ui within the render method) */}
          <div className={`col thickness-height`}>
            <p
              style={{ fontSize: '35px', paddingTop: '40px' }}
            ...
            <Slider
              ...
              max={30}
              onChange={(_, newValue) =>
                (canvasData.freeDrawingBrush.width = newValue)
              }
            />
          </div>
        </div>
